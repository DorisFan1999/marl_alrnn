...
class TimeSeriesDataset:
    ...
    def __init__(self, data, external_inputs=None, sequence_length=200, batch_size=16, target_data=None):
        """
        target data: fully 6-dimensional action probability, shape (n_trajectories, n_steps, 6), which is used-
        for modified ALRNN training, for computation of loss function. 
        """
        self.X = torch.tensor(data, dtype=torch.float32)
        self.Y = self.X if target_data is None else torch.tensor(target_data, dtype=torch.float32)

        if self.X.ndim != 3:
            raise ValueError("data must have dimension=3")
        if self.Y.ndim != 3:
            raise ValueError("target_data must have dimension=3")
        if self.Y.shape[:2] != self.X.shape[:2]:
            raise ValueError("target_data must match data on trajectory and time axes")
    ...
    def __getitem__(self, idx):
        ...
        y = self.Y[trajectory_idx, t + 1:t + self.sequence_length + 1, :]
    ...
    def sample_batch_with_state_target(self):
        """Sample inputs with next-step state and output targets."""
        X, X_target, Y, S = [], [], [], []
        ...
        for _ in range(self.batch_size):
            ...
            x_target = self.X[
                trajectory_idx,
                t + 1:t + self.sequence_length + 1,
                :,
            ]
            X.append(x)
            X_target.append(x_target)
            Y.append(y)
            S.append(s)
        ...
        if S[0] is None:
            return X, X_target, Y, None

        return X, X_target, Y, torch.stack(S)
    ...
    def reference_target_sequence(self, trajectory_idx=0, max_length=None):
        """Return one full target trajectory for diagnostics."""
        if not 0 <= trajectory_idx < self.n_trajectories:
            raise IndexError("trajectory_idx is out of range")

        seq = self.Y[trajectory_idx]
